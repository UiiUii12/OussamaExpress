...
//panier
  ...
  Stream<List<Plat>> get favoris {
    return clientCollection
        ...
        .collection("Favoris")
        .snapshots()
        .map((snapshot) => _favorisList(snapshot));
  }
    ...
    clientCollection.doc(uid).get().then((value) {
      if (!value.exists) {
        clientCollection.doc(uid).set(
            {"nom": "new user", "panier": 0.toInt(), "favoris": 0.toInt()});
      }
    });
  }
  ...
  Stream<int> get existfavoris {
    return clientCollection
        .doc(uid)
        .snapshots()
        .map((snapshot) => _existfavoris(snapshot));
  }
...
//********************************************************************************************
  ...
  UpdateFavorisPlus() async {
    await clientCollection
        .doc(uid)
        .get()
        .then((value) => nbrFavoris = value.get("favoris").toInt());
